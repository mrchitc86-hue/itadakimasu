...
- **基本記事**（Start here）：朝ごはん・コンビニ・おにぎりのような、長く読まれる説明。`index.html` の「Start here」のカードに並ぶ。
- **Now in Japan**（時事記事）：日付つき。いま日本で起きていることを、企業発表や報道をもとに書く。`index.html` の「Now in Japan」に並ぶ。古くなったら日付が示してくれるので、消さずに残してよい。
  - 事実は必ず出典（企業の発表ページなど）にリンクする
  - 報道の文章をそのまま写さない。自分の言葉で要約する
  - 「Written 10 September 2026」のように書いた日を入れる
